...
// Map enhancement for MiniScript methods
function enhanceMap(map) {
    if (typeof map !== 'object' || map === null || Array.isArray(map)) return map;
    // Only define properties if not already present
    if (!Object.getOwnPropertyDescriptor(map, 'hasIndex')) {
    ...
    window.miniscript = {
        ...
        Interpreter: function() {
            ...
            this.variables = {
                ...
                }            };            // Convert arrays and maps to MiniScript-style string representation
            ...
            this.run = function(code) {
                ...
                while (i < lines.length) {
                    ...
                    if (line.startsWith("for ")) {
                        ...
                        const forMapMatch = line.match(/^for\s+(\w+)\s*,\s*(\w+)\s+in\s+(.+)$/);
                        if (forMapMatch) {
                            const keyVarName = forMapMatch[1];
                            const valueVarName = forMapMatch[2];
                            const mapExpr = forMapMatch[3];
                            
                            // Evaluate the map expression
                            const map = this.evalExpr(mapExpr);
                            
                            // Check if the result is an object (map)
                            if (typeof map === 'object' && map !== null && !Array.isArray(map)) {
                                ...
                                // Execute loop for each key-value pair
                                const keys = Object.keys(map);
                                for (const key of keys) {
                                    this.variables[keyVarName] = key;
                                    this.variables[valueVarName] = map[key];
                                    this.run(loopLines.join('\n'));
                                }
                        ...
                        if (forListMatch) {
                            ...
                            if (Array.isArray(list)) {
                                ...
                                // Handle map with single variable (keys only)
                                enhanceMap(list);
                                let loopLines = [];
                                i++;
                                ...
                                // Execute loop for each key in the map
                                const keys = Object.keys(list);
                                for (const key of keys) {
                                    this.variables[itemVarName] = key;
                                    this.run(loopLines.join('\n'));
                                }
                                ...
                                console.error("Expected a list or map in for loop, got:", typeof list);
                                // Skip to end for
                                while (i < lines.length && !lines[i].trim().startsWith("end for")) {
                                    i++;
                                }
                            ...
                            if (leftSide.includes('[') && leftSide.includes(']')) {
                                ...
                                if (this.variables.hasOwnProperty(containerName)) {
                                    ...
                                    if (Array.isArray(container)) {
                                        ...
                                        enhanceMap(container); // Ensure map has its methods
            ...
            this.evalExpr = function(expr, depth = 0) {
                ...
                if (expr.startsWith('{') && expr.endsWith('}')) {
                    ...
                    for (let entry of entries) {
                        // Split on the first colon outside of quotes, brackets, etc.
                        let inStr = false;
                        let brackD = 0, pareD = 0, bracD = 0;
                        let colonPos = -1;
                        ...
                        for (let i = 0; i < entry.length; i++) {
                            ...
                            if (c === '"' && (i === 0 || entry[i-1] !== '\\')) {
                                ...
                                else if (c === ':' && brackD === 0 && pareD === 0 && bracD === 0) {
                                    colonPos = i;
                                    break;
                                }
                            }
                        ...
                        if (colonPos > 0) {
                            const keyStr = entry.substring(0, colonPos).trim();
                            const valueStr = entry.substring(colonPos + 1).trim();
                            
                            // Parse key (removing quotes if string key)
                            let key = keyStr;
                            ...
                            // Evaluate value
                            const value = this.evalExpr(valueStr, depth + 1);
                            result[key] = value;
                        }
                    }
                    
                    return enhanceMap(result);
                }
                    ...
                    if (openBracketForFinalIndex !== -1) { 
                        ...
                        if (baseExprStr !== "") {
                            ...
                            if (Array.isArray(baseVal)) {
                                ...
                                enhanceMap(baseVal); // Ensure map has its methods
                                indexedOnceVal = baseVal[String(indexVal)];
                                if (indexedOnceVal === undefined) {
                                     return null; // In MiniScript, accessing a non-existent map key returns null
                                }
                ...
                */
                  // Handle list/map method calls with chaining: baseExpr.method(args)
                // Supports chaining like list.push(1).push(2) or map.remove(key).hasIndex(key)
                let methodCallMatch = expr.match(/^(.*?)(?:\.(pop|pull|push|hasIndex|indexes|values|remove|clear)\(([^)]*)\))+$/);
                if (methodCallMatch) {
                    // Split the expression into base and method calls
                    // We'll use a regex to extract all method calls in order
                    let baseExpr = expr;
                    let methodCalls = [];
                    let methodCallRegex = /\.(pop|pull|push|hasIndex|indexes|values|remove|clear)\(([^)]*)\)/g;
                    let m;
                    ...
                    for (let call of methodCalls) {
                        ...
                        if (Array.isArray(value)) {
                            ...
                            if (call.method === "push") {
                                ...
                                console.error("Method '" + call.method + "' is not a valid list method");
                                return null;
                            }
                        }
                        // Map methods
                            ...
                            enhanceMap(value);
                            if (call.method === "hasIndex") {
                                const arg = call.argStr.length ? this.evalExpr(call.argStr, depth + 1) : undefined;
                                value = value.hasIndex(arg);
                            } else if (call.method === "indexes") {
                                value = enhanceList(value.indexes());
                            } else if (call.method === "values") {
                                value = enhanceList(value.values());
                            } else if (call.method === "remove") {
                                const arg = call.argStr.length ? this.evalExpr(call.argStr, depth + 1) : undefined;
                                value = value.remove(arg);
                            } else if (call.method === "clear") {
                                value = value.clear();
                            } else {
                                console.error("Method '" + call.method + "' is not a valid map method");
                                return null;
                            }
                        } else {
                            console.error("Method '" + call.method + "' called on invalid type: " + typeof value);
                            return null;
                        }
                    }
                    return value;
